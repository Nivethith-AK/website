...
export async function POST(req: NextRequest) {
  try {
    ...
    if (!file) {
      return NextResponse.json({ success: false, message: "profileImage is required" }, { status: 400 });
    }

    const ext = (file.name.split(".").pop() || "png").toLowerCase();
    const path = `designers/${profile.id}/avatar-${Date.now()}.${ext}`;
    const fileUrl = await uploadToBucket("aurax-files", path, file);

    const supabase = getSupabaseAdmin();
    const { data, error } = await supabase
      .from("profiles")
      .update({ profile_image: fileUrl, updated_at: new Date().toISOString() })
      .eq("id", profile.id)
      .select("*")
      .single();
